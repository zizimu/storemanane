...
<body>
	<form class="form-inline definewidth m20"
		action="${pageContext.request.contextPath}/Stock/s" method="get">
		商品编号： <input type="text" name="wd" id="wd"
			class="abc input-default" placeholder="商品编号：">&nbsp;&nbsp; 
			...
			<c:forEach items="${stocksList}" var="p" varStatus="st">
				<tr>
					...
					<td style="text-align: center"><a
						href="${pageContext.request.contextPath}/Stock/edit?goodsId=${p.goodsId}">
							<button class="btn btn-primary" style="margin-left: 5px;">
								编辑</button>
					</a>
						<button class="btn btn-success" style="margin-left: 5px;"
							onclick="del(${p.batchId},${p.goodsId})">删除</button></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
	<div class="inline pull-right page">
		...
		<c:choose>
			<c:when test="${wd!=null}">
				<a
					href="${pageContext.request.contextPath}/Stock/s?wd=${wd}&page=${page.firstPage}">第一页</a>
				<c:choose>
					<c:when test="${!page.isFirstPage}">
						<a
							href="${pageContext.request.contextPath}/Stock/s?wd=${wd}&page=${page.prePage}">上一页</a>
					</c:when>
				</c:choose>
				<c:choose>
					<c:when test="${!page.isLastPage}">
						<a
							href="${pageContext.request.contextPath}/Stock/s?wd=${wd}&page=${page.nextPage}">下一页</a>
					</c:when>
				</c:choose>
				<a
					href="${pageContext.request.contextPath}/Stock/s?wd=${wd}&page=${page.lastPage}">最后页</a>
			</c:when>
			<c:otherwise>
				<a
					href="${pageContext.request.contextPath}/Stock?page=${page.firstPage}">第一页</a>
				<c:choose>
					<c:when test="${!page.isFirstPage}">
						<a
							href="${pageContext.request.contextPath}/Stock?page=${page.prePage}">上一页</a>
					</c:when>
				</c:choose>
				<c:choose>
					<c:when test="${!page.isLastPage}">
						<a
							href="${pageContext.request.contextPath}/Stock?page=${page.nextPage}">下一页</a>
					</c:when>
				</c:choose>
				<a
					href="${pageContext.request.contextPath}/Stock?page=${page.lastPage}">最后页</a>
			</c:otherwise>
		</c:choose>
	</div>
</body>
