...
<body>
				...
				<c:forEach items="${accounts}" var="p" varStatus="st">
					<tr>
						...
						<c:if test="${p.status eq 0 }">
							<td style="text-align: center;color: green">审核中</td>
						</c:if>
						<c:if test="${p.status eq 2 }">
							<td style="text-align: center;color: green">分店权限</td>
						</c:if>
						<c:if test="${p.status eq 9 }">
							<td style="text-align: center;color: green">总店权限</td>
						</c:if>
						<%-- <td style="text-align: center; color: green">${p.status}</td> --%>
						
							<!-- <td style="text-align: center">分店</td> -->
							<td style="text-align: center">${stores[p.storeid]}</td>
							<%-- <td style="text-align: right">${p.mark}</td> --%>
							<td style="text-align: center"><a
								href="${pageContext.request.contextPath}/account/edit/${p.sid}">
	...
	<div class="inline pull-right page" style="margin-top: 30px;">
		总共${page.total}条记录 第 ${page.pageNum}/${page.pages} 页 <a
			href="${pageContext.request.contextPath}/account?page=${page.firstPage}">第一页</a>
		<c:choose>
			<c:when test="${!page.isFirstPage}">
				<a
					href="${pageContext.request.contextPath}/account?page=${page.prePage}">上一页</a>
			</c:when>
		</c:choose>
		<c:choose>
			<c:when test="${!page.isLastPage}">
				<a
					href="${pageContext.request.contextPath}/account?page=${page.nextPage}">下一页</a>
			</c:when>
		</c:choose>
		<a
			href="${pageContext.request.contextPath}/account?page=${page.lastPage}">最后页</a>
	</div>
</body>
</html>
